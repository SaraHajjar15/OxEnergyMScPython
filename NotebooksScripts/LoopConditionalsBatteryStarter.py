...
for i in range(len(demand_P)): # <--- Fill in the for loop to iterate over the range of demand_P length
    ...
    # ITS WRONG SEE PICTURE 27 october 2025
    if i == 0:
        ...
        if soc_E[i] > max_soc:
            soc_E[i] = max_soc     
        print("state of charge is for the",i ,"step is", soc_E[i])
    

    # update the net demand after battery operation
    net_demand_P[i] = demand_P[i] + bat_P[i]  # Net demand is original demand plus battery power
    print("demand is", demand_P[i])
    print("battery is", bat_P[i])

# Print the final state of charge
print(net_demand_P) # <--- Add a print statement to display the final state of charge. 
print("Final State of Charge:", soc_E[-1])
print("Battery state of charge over time:", soc_E)